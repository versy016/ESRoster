...
export async function generateRosterHTML(roster, surveyors, area, anchorDate) {
  ...
  // Load STSP roster
  const stspRoster = await loadRosterForDate(startDate, "SOUTH");
  ...
  // Load NTNP roster
  const ntnpRoster = await loadRosterForDate(startDate, "NORTH");
...
export async function exportRosterToPDFWeb(roster, surveyors, area, anchorDate) {
  ...
  return new Promise((resolve, reject) => {
    printWindow.onload = () => {
      setTimeout(() => {
        printWindow.print();
        // Don't close the window immediately - let user save as PDF
