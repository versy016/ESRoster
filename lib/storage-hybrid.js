...
export async function loadRosterForDate(date, area = "SOUTH", role = null) {
    ...
    try {
      ...
      if (result.success) {
        const matchingRoster = result.data.find((r) => {
          ...
          // For surveyors, filter out draft rosters (only show confirmed/published)
          // Also filter out rosters without a status (treat null/undefined as draft)
          if (role === "surveyor") {
            const rosterStatus = r.status || "draft";
            if (rosterStatus === "draft" || !r.status || r.status === null) {
              return false;
            }
          }
  ...
  try {
    ...
    const matchingRoster = rosters.find((r) => {
      ...
      // For surveyors, filter out draft rosters (only show confirmed/published)
      if (role === "surveyor" && (r.status === "draft" || !r.status)) {
        console.log(`Roster ${r.id}: Draft roster - hidden from surveyors (AsyncStorage fallback)`);
        return false;
      }
