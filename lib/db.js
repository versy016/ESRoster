...
export async function updateSurveyor(id, updates) {
  ...
  // Only update photo_url if it's provided (allows updating to null/empty)
...
export async function createRoster(roster) {
  ...
  // Include disabled_rules if provided
  if (roster.disabledRules !== undefined) {
    insertData.disabled_rules = JSON.stringify(roster.disabledRules);
  }
...
export async function updateRoster(id, updates) {
  ...
  if (updates.disabledRules !== undefined) {
    updateData.disabled_rules = JSON.stringify(updates.disabledRules);
  }
  const { data, error } = await supabase
    .from("rosters")
    .update(updateData)
...
// ==================== USER-SURVEYOR LINKING ====================

/**
 * Automatically link a user to a surveyor by matching email addresses
 * Uses Edge Function (server-side) for security, falls back to client-side if unavailable
 * @param {string} userId - The authenticated user's ID
 * @param {string} userEmail - The user's email address
 * @param {boolean} useEdgeFunction - Whether to use Edge Function (default: true)
 * @returns {Promise<{success: boolean, surveyorId?: string, error?: string}>}
 */
export async function autoLinkUserToSurveyorByEmail(userId, userEmail, useEdgeFunction = true) {
  try {
    console.log(`[AUTO-LINK] Attempting to link user ${userId} with email ${userEmail} to surveyor`);

    if (!userEmail || !userId) {
      return { success: false, error: 'Email and userId are required' };
    }
    ...
    if (useEdgeFunction) {
      try {
        ...
        if (error) {
          console.warn('[AUTO-LINK] Edge Function error, falling back to client-side:', error);
          // Fall through to client-side
        } else if (data) {
          if (data.success) {
            console.log(`[AUTO-LINK] Successfully linked via Edge Function:`, data);
            return {
              success: true,
            ...
            // Edge Function returned failure (e.g., no match found)
            return {
              success: false,
        ...
        console.warn('[AUTO-LINK] Edge Function not available, using client-side linking:', edgeError);
      }
    }

    // Fallback to client-side linking (for development or if Edge Function unavailable)
    // Find surveyor with matching email (case-insensitive)
    const normalizedEmail = userEmail.trim().toLowerCase();
    const { data: surveyors, error: findError } = await supabase
      .from("surveyors")
    ...
    if (findError) {
      console.error("[AUTO-LINK] Error finding surveyor by email:", findError);
      return { success: false, error: findError.message };
    }

    // Check if any surveyor was found
    if (!surveyors || surveyors.length === 0) {
      console.log(`[AUTO-LINK] No surveyor found with email ${userEmail}`);
      return { success: false, error: 'No surveyor found with matching email' };
    }

    const surveyor = surveyors[0];

    // Check if surveyor is already linked to a different user
    if (surveyor.user_id && surveyor.user_id !== userId) {
      console.warn(`[AUTO-LINK] Surveyor ${surveyor.id} is already linked to user ${surveyor.user_id}`);
      return {
        success: false,
        error: `Surveyor is already linked to another user`,
        surveyorId: surveyor.id
      };
    }

    // Check if surveyor is already linked to this user
    if (surveyor.user_id === userId) {
      console.log(`[AUTO-LINK] Surveyor ${surveyor.id} is already linked to user ${userId}`);
      return { success: true, surveyorId: surveyor.id, alreadyLinked: true };
    }

    // Link the user to the surveyor
    ...
    if (linkError) {
      console.error("[AUTO-LINK] Error linking user to surveyor:", linkError);
      return { success: false, error: linkError.message };
    }

    console.log(`[AUTO-LINK] Successfully linked user ${userId} to surveyor ${surveyor.id} (${surveyor.name})`);
    return { success: true, surveyorId: surveyor.id, data };
  } catch (error) {
    console.error("[AUTO-LINK] Exception linking user to surveyor:", error);
    return { success: false, error: error.message };
  }
}

/**
 * Link a user to a surveyor profile
 * @param {string} userId - The authenticated user's ID
 * @param {string} surveyorId - The surveyor's ID to link
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function linkUserToSurveyor(userId, surveyorId) {
  try {
    console.log(`[LINK USER] Linking user ${userId} to surveyor ${surveyorId}`);

    // Update surveyors table to link user_id
    const { data, error: surveyorError } = await supabase
      .from("surveyors")
      .update({ user_id: userId })
      .eq("id", surveyorId)
    ...
    if (surveyorError) {
      console.error("[LINK USER] Error linking user to surveyor:", surveyorError);
      return { success: false, error: surveyorError.message };
    }

    // Verify the update worked
    if (!data || data.length === 0) {
      console.error("[LINK USER] Update returned no data - surveyor may not exist");
      return { success: false, error: "Surveyor not found" };
    }

    console.log(`[LINK USER] Successfully linked user ${userId} to surveyor ${surveyorId}. Updated surveyor:`, data[0]);
    return { success: true, data: data[0] };
  } catch (error) {
    console.error("[LINK USER] Exception linking user to surveyor:", error);
    return { success: false, error: error.message };
  }
}

/**
 * Unlink a user from their surveyor profile
 * @param {string} userId - The authenticated user's ID
 * @returns {Promise<{success: boolean, error?: string}>}
 */
export async function unlinkUserFromSurveyor(userId) {
  try {
    // Update surveyors table to remove user_id
    ...
    if (surveyorError) {
      console.error("Error unlinking user from surveyor:", surveyorError);
      return { success: false, error: surveyorError.message };
    }

    return { success: true };
  } catch (error) {
    console.error("Error unlinking user from surveyor:", error);
    return { success: false, error: error.message };
  }
}

/**
 * Get user profile with role and surveyor link
 * @param {string} userId - The authenticated user's ID
 * @returns {Promise<{success: boolean, data?: object, error?: string}>}
 */
export async function getUserProfile(userId) {
  try {
    // Get surveyor record linked to this user
    const { data, error } = await supabase
      .from("surveyors")
      .select("*")
      .eq("user_id", userId)
    ...
    // Return the surveyor data as the user profile
    // The surveyor record contains user_id and role
    return { success: true, data };
  } catch (error) {
    console.error("Error fetching user profile:", error);
    return { success: false, error: error.message, data: null };
